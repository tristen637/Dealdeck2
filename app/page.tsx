  ...
  return (
    <div>
      ...
      <div style={{ background: '#fff', border: '1px solid #E5E7EB', borderRadius: 14, overflow: 'hidden' }}>
        <div style={{ padding: '12px 16px', background: '#111827', color: '#F9FAFB', fontWeight: 700, fontSize: 14 }}>
          Recent Signups
        </div>
        {stats.recentSignups.length === 0 && (
          <div style={{ padding: 20, textAlign: 'center', color: '#9CA3AF', fontSize: 14 }}>No signups yet</div>
        )}
          ...
          <div key={i} style={{ padding: '12px 16px', borderBottom: '1px solid #F3F4F6', display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 8 }}>
            <div>
              ...
              <div style={{ fontSize: 12, color: '#9CA3AF' }}>
                ...
                {u.trialEnd && u.status === 'Trial' && ` · Trial ends ${new Date(u.trialEnd).toLocaleDateString()}`}
              </div>
            </div>
            <span style={{
              background: u.status === 'Pro' ? '#ECFDF5' : u.status === 'Trial' ? '#FFFBEB' : '#F9FAFB',
              color: u.status === 'Pro' ? '#065F46' : u.status === 'Trial' ? '#92400E' : '#6B7280',
              border: `1px solid ${u.status === 'Pro' ? '#6EE7B7' : u.status === 'Trial' ? '#FDE68A' : '#E5E7EB'}`,
    ...
    <div style={{ minHeight: '100vh', background: '#F9FAFB' }}>
      <nav style={{ background: '#111827', padding: '0 20px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', height: 56, position: 'sticky', top: 0, zIndex: 50 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          {isAdmin && (
            <button onClick={() => setView(view === 'admin' ? 'tool' : 'admin')} style={{ background: view === 'admin' ? '#10B981' : '#1F2937', color: '#fff', border: 'none', borderRadius: 8, padding: '6px 14px', fontSize: 13, fontWeight: 700, cursor: 'pointer' }}>
              {view === 'admin' ? '← Back to Tool' : '📊 Admin'}
            </button>
          )}
